...
def manifestVersion(boolean concatenateBranchNameAsSuffix = true) {
	...
	else if (isSignificantVersion(currentVersionName) == true) {
		manifestedVersion = "${currentVersionName}${versionSuffix}"
	}
...
def manifestNamespace() {
	...
	// If we are on the production or staging branches return the regular name ($service_name), else return the branch name itself
